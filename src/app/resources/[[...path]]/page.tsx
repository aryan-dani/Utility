...
export default function ResourcesPage() {
  ...
  useEffect(() => {
    ...
    async function loadResources() {
      ...
      if (!cancelled) {
        setResources(data);
        setSelectedSubject(null); // reset on branch/sem change
      }
  ...
  // Group by subject — stable with useMemo to avoid dependency loop
  ...
  useEffect(() => {
    if (!loading && subjectNames.length > 0 && !selectedSubject) {
      setSelectedSubject(subjectNames[0]);
    }
  }, [loading, subjectNames, selectedSubject]);

  // Search filter — applied to the currently visible subject's resources
  ...
  // When searching, also filter the sidebar subject list
